...
export interface CommandPermissions {
 ...
 cache: Discord.Collection<
  string,
  Discord.Collection<string, Discord.ApplicationCommandPermissions[]>
 >;
}
...
const self: CommandPermissions = {
 get: async (guild, commandId) => {
  ...
  const requestHandler = (await import('./requestHandler.js')).default;
 ...
 set: (guildId, commandId, permissions) => {
  if (!self.cache.get(guildId)) self.cache.set(guildId, new Discord.Collection());
