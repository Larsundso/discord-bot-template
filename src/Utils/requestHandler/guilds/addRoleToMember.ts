...
export default async (guild: Discord.Guild, userId: string, roleId: string, reason?: string) => {
 ...
 if (!canAddRoleToMember(roleId, guild.members.me)) {
 ...
 return API.guilds.addRoleToMember(guild.id, userId, roleId, { reason }).catch((e) => {
 ...
 */
export const canAddRoleToMember = (roleId: string, me: Discord.GuildMember | null) =>
 me &&
