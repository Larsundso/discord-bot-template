...
export default async (message: Discord.Message<true>) => {
 ...
 if (!canDeleteAllReactions(message.channel.id, message.guild.members.me)) {
 ...
 return API.channels.deleteAllMessageReactions(message.channel.id, message.id).catch((e) => {
 ...
 */
export const canDeleteAllReactions = (channelId: string, me: Discord.GuildMember | null) =>
